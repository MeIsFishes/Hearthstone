...
namespace BbxCommon.Ui
{
    public static class UiApi
    {
        ...
        /// </summary>
        public static IReadOnlyDictionary<string, string> CapturePreloadedUiPrefabPathsForValidation()
        {
            var data = Resources.Load<PreLoadUiData>(BbxVar.ExportPreLoadUiPathInResources);
            if (data == null)
                throw new InvalidOperationException("PreLoadUiData asset is missing.");
            var result = new Dictionary<string, string>(StringComparer.Ordinal);
            foreach (var pair in data.UiDatas)
                result.Add(pair.Key, pair.Value.UiViewPrefabPath ?? string.Empty);
            return result;
        }
#endif
        #endregion

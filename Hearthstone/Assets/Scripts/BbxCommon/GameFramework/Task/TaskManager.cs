...
namespace BbxCommon
{
    internal class TaskManager : Singleton<TaskManager>
    {
        ...
        internal List<TaskBase> CurrentTickEnterTasks = new();
        ...
        internal void RunTaskInCurrentTick(TaskBase task)
        {
            CurrentTickEnterTasks.Add(task);
        }

        internal TaskBase CreateTask(string key, TaskContextBase context, bool run)
        {
            ...
            if (taskGroupInfo.RootTaskId < 0 || taskGroupInfo.RootTaskId >= taskList.Count)
            {
                ...
                for (var i = 0; i < taskList.Count; i++)
                    taskList[i]?.CollectToPool();
                taskList.CollectToPool();
                return null;
            }
            var root = taskList[taskGroupInfo.RootTaskId];
            root.OwnTaskInstances(taskList);
            root.SourceTaskKey = key;
            if (run)
            {
                RunTask(root);
            }

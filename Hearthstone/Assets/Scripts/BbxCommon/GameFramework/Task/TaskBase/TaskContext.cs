...
namespace BbxCommon
{
    public abstract class TaskContextBase : PooledObject
    {
        ...
        {
            if (m_blackBoardLongData.ContainsKey(key))
            {
        ...
        {
            if (m_blackBoardDoubleData.ContainsKey(key))
            {
        ...
        /// <summary>
        /// Applies a typed, CSV-backed collection of initial blackboard values.
        /// </summary>
        public void ApplyBlackBoardInjection(TaskBlackboardInjection injection)
        {
            if (injection == null)
                return;

            for (var i = 0; i < injection.Count; i++)
            {
                var entry = injection[i];
                switch (entry.ValueType)
                {
                    ...
                    case ETaskBlackboardInjectionValueType.Long:
                        SetBlackBoardLongValue(entry.Key, entry.LongValue);
                        break;
                    case ETaskBlackboardInjectionValueType.Float:
                    case ETaskBlackboardInjectionValueType.Double:
                        SetBlackBoardDoubleValue(entry.Key, entry.DoubleValue);
                        break;
                    case ETaskBlackboardInjectionValueType.String:
                        SetBlackBoardObjectValue(entry.Key, entry.StringValue);
                        break;
                    default:
        ...
        /// <summary>
        /// Copies the current blackboard snapshot to another task context.
        /// Object values are copied by reference; combat blackboards only register immutable values and value objects.
        /// </summary>
        public void CopyBlackBoardTo(TaskContextBase target)
        {
            if (target == null)
                throw new ArgumentNullException(nameof(target));

            foreach (var pair in m_blackBoardLongData)
                target.SetBlackBoardLongValue(pair.Key, pair.Value);
            foreach (var pair in m_blackBoardDoubleData)
                target.SetBlackBoardDoubleValue(pair.Key, pair.Value);
            foreach (var pair in m_blackBoardObjectData)
                target.SetBlackBoardObjectValue(pair.Key, pair.Value);
        }
        ...
        protected void RegisterObject<TEnum, TObject>(TEnum fieldEnum, TObject value) where TEnum : Enum
        {
            ...
            if (contextData.Inited == false)
            {
                contextData.FieldTypeDic[fieldEnum.ToString()] = typeof(TObject);
                contextData.FieldStrIndexDic[fieldEnum.ToString()] = enumValue;
            }
            m_FieldList[enumValue] = new RegisteredField(value);
        }
        ...
        /// <summary>Reserved index: Context field name was empty in export; do not pass to GetConstValue.</summary>
        internal const int UnboundContextFieldIndex = int.MaxValue;

        internal int GetStrIndex(string str)
        {
            if (string.IsNullOrEmpty(str))
                return UnboundContextFieldIndex;

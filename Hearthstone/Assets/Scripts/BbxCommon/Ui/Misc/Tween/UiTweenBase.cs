...
namespace BbxCommon.Ui
{
    ...
    public abstract class UiTweenBase: MonoBehaviour, IUiPreInit, IUiInit, IUiOpen, IUiShow, IUiUpdate, IUiHide, IUiClose, IUiDestroy
    {
        ...
        #region Helpers
        /// <summary>
        /// Call after dynamically adding a component as a tween target at runtime.
        /// If the owning UI controller has already passed Init/Open/Show, the new component
        /// will receive the corresponding callbacks immediately so it is fully initialized.
        /// </summary>
        protected void EnableNewUiItem(Component item)
        {
            var view = GetComponentInParent<UiViewBase>();
            if (view != null && view.UiController != null)
                view.UiController.EnableUiItem(item);
        }
        #endregion
